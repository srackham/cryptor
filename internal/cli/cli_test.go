...
func TestParseArgs(t *testing.T) {
	...
	parse("cryptor valuate -confdir")
	assert.Equal(t, "missing -confdir argument value", err.Error())
	parse("cryptor valuate -currency")
	assert.Equal(t, "missing -currency argument value", err.Error())
	parse("cryptor valuate -format")
	assert.Equal(t, "missing -format argument value", err.Error())
	parse("cryptor valuate -portfolio")
	assert.Equal(t, "missing -portfolio argument value", err.Error())
	parse("cryptor valuate -price")
	assert.Equal(t, "missing -price argument value", err.Error())
	parse("cryptor valuate -format xml")
	assert.Equal(t, `invalid -format argument: "xml"`, err.Error())
	parse("cryptor valuate -portfolio invalid@name")
	assert.Equal(t, `invalid -portfolio argument: "invalid@name"`, err.Error())
	parse("cryptor valuate -portfolio dup -portfolio dup")
	assert.Equal(t, `-portfolio name can only be specified once: "dup"`, err.Error())
	parse("cryptor valuate -price INVALID")
	assert.Contains(t, err.Error(), `invalid price option: "INVALID"`)
	parse("cryptor valuate -invalid-opt")
	assert.Equal(t, `invalid argument: "-invalid-opt"`, err.Error())
	parse("cryptor valuate -portfolio valid -invalid")
	assert.Equal(t, `invalid argument: "-invalid"`, err.Error())
}
